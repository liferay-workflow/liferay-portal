	...
	<c:otherwise>
		...
		<c:if test="<%= supportedActions.contains(ActionKeys.VIEW) %>">
			<p>
				...
				<div class="mt-3" style="margin-bottom: 60px" <%= inputPermissionsShowOptions ? "hide" : "" %>" id="<%= uniqueNamespace %>inputPermissionsShowOptionsLink">
					<a class="btn btn-secondary btn-sm" role="button" href="javascript:<%= uniqueNamespace %>inputPermissionsShowOptions();"><liferay-ui:message key="more-options" /></a> <liferay-ui:icon-help message="input-permissions-more-options-help" />
				</div>
