...
public class WorkflowTaskManagerImplTest extends BaseWorkflowManagerTestCase {
	...
	@Test
	public void testGetNotifiableUsersCommerceAsAccountAdmin()
		throws Exception {

		String userAccountAdminEmail =
			StringUtil.toLowerCase(RandomTestUtil.randomString()) +
				RandomTestUtil.nextLong() + "@liferay.com";

		User userAccountAdmin = UserTestUtil.addUser(
			_company.getCompanyId(), _adminUser.getUserId(), StringPool.BLANK,
			userAccountAdminEmail,
			RandomTestUtil.randomString(
				NumericStringRandomizerBumper.INSTANCE,
				UniqueStringRandomizerBumper.INSTANCE),
			LocaleUtil.getDefault(), RandomTestUtil.randomString(),
		...
		CommerceAccountTestUtil.addAccountEntryUserRels(
			...
			new long[] {userAccountAdmin.getUserId()},
			ServiceContextTestUtil.getServiceContext());

		Role roleAccountAdmin = RoleLocalServiceUtil.getRole(
			_company.getCompanyId(),
			AccountRoleConstants.REQUIRED_ROLE_NAME_ACCOUNT_ADMINISTRATOR);

		UserGroupRoleLocalServiceUtil.addUserGroupRoles(
			userAccountAdmin.getUserId(), accountEntry.getAccountEntryGroupId(),
			new long[] {roleAccountAdmin.getRoleId()});
		...
		List<User> users = ListUtil.filter(
			...
			user -> StringUtil.equals(
				userAccountAdminEmail, user.getEmailAddress()));
